...
def test_load_ngspice_variables():
    ...
    correct_result_file = "tests/result_skywater_char_variables.txt"
    ...
    with contextlib.redirect_stdout(print_output):
        spyci.list_vars("tests/skywater_char_vgs.raw")
...
def test_load_ngspice_content():
    ...
    # Load the correct answer
    correct_result_file = "tests/result_skywater_char_values.txt"
    correct_result_values = open(correct_result_file, "r").read().strip('\n')

    data = spyci.load_raw("tests/skywater_char_vgs.raw")
